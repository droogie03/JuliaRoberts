...
bot.on('text', function(data){
    if(data.text.match('si') && isQuestion){
        isQuestion = false;
        return bot.sendMessage(data.chat.id, CONSTANTS_TEXT.FIRST_ANSWER_TEXT);
    }else if(isQuestion){
        return bot.sendMessage(data.chat.id, CONSTANTS_TEXT.FIRST_WRONT_ANSWER);
    }
})

bot.on(['audio', 'sticker'], msg => {
    return bot.sendMessage(msg.from.id, 'Lo siento estoy muy ocupada y no tengo datos, asiq no puedo oir ni ver audios ni stickers sorry.');
  });
