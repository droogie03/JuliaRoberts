...
bot.on("text", function (data) {
  bot.sendMessage(data.chat.id, data.chat.id);
  bot.sendMessage(data.chat.id, data.chat.text);
...
bot.on(["audio", "sticker"], (msg) => {
  bot.sendMessage(
    msg.from.id,
    "Lo siento estoy muy ocupada y no tengo datos, asiq no puedo oir ni ver audios ni stickers sorry."
  );
});
